...
<p style="margin-top:8px"><b style="color:var(--ink)">Use of GenAI:</b> GitHub Copilot assisted with brainstorming, application-structure advice, drafting the JIRA epics &amp; user stories, Figma guidance, generating &amp; auto-completing code snippets inside VS Code, generating the automated test suites (Mocha + Sinon backend tests and the React Testing Library frontend test), and helping build the CI/CD pipeline; all output was reviewed, run and adapted.</p>
</div></footer>
